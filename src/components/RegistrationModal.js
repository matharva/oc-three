...
const MultiplePlayers = ({ setPayTypeOpen,eventData,currentUser, setJoin,setIsOpen }) => {
  console.log("ksejygk");
  ...
  const joinUser = async() =>{
    ...
    let joinData = await eventService.joinUser({teamCode:code,userId:currentUser.uid,eventName:eventData.Title})
    ...
    <>
      <div className="modal-grid">
        <div className="section-one">
          <h1>Join a team</h1>
          <input type="text" placeholder="Enter your team code..." onChange = {(e)=>{
...
const SinglePlayer = ({ setPaymentDone, setIsPaymentSuccess, eventData, currentUser }) => {
  ...
  return (
    <div className="modal-grid-one">
      ...
      <button onClick = {()=>{showRazorpayModal(eventData.Fee[0].Fee)}}>Pay</button>
    </div>
  );
};
...
const ShowPaymentDetails = ({details,eventName,setPaymentDone,setIsPaymentSuccess,currentUser}) =>{
...
const RegistrationModal = ({ userTeam,viewTeam, setViewTeam, isOpen, setIsOpen,eventData,paymentDone, setPaymentDone,currentUser,setJoin }) => {
  ...
  return (
    <div>
      ...
      >
        ...
        {viewTeam?<TeamModal userTeam = {userTeam}/>:isPaymentSuccess?<PaymentSuccessful eventData = {eventData} setViewTeam={setViewTeam} />:!isRegistered && eventData.isSingle? <SinglePlayer setPaymentDone = {setPaymentDone} setIsPaymentSuccess = {setIsPaymentSuccess} eventData = {eventData}  currentUser = {currentUser}/> : !payTypeOpen?<MultiplePlayers setPayTypeOpen = {setPayTypeOpen} eventData = {eventData} currentUser = {currentUser} setJoin = {setJoin} setIsOpen = {setIsOpen}/>:<ShowPaymentDetails details={eventData.Fee} eventName={eventData.Title} setPaymentDone = {setPaymentDone} setIsPaymentSuccess = {setIsPaymentSuccess} currentUser = {currentUser}/>}
